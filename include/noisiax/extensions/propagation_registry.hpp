...
using PropagationFn = std::function<void(double& target, const double& source, double weight)>;

class PropagationFunctionRegistry {
public:
    void register_function(const std::string& symbol_id, PropagationFn fn) {
    ...
    const PropagationFn* find(std::string_view symbol_id) const {
    ...
    const std::map<std::string, PropagationFn>& functions() const { return functions_; }
    const std::map<std::string, std::string>& aliases() const { return aliases_; }

private:
    std::map<std::string, PropagationFn> functions_;
